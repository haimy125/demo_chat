...
io.on('connection', function(socket) { //neu co thi goi function ket noi. khi do server se tao ra bien socket de quan ly ket noi voi 1 doi tuong cu the. cu 1 nguoi truy cap se tao ra 1 bien socket cu the, quan ly ho. nho socket nen server phan biet duoc ai voi ai. va trong socket co nhieu thuoc tinh lam.
    ...
    socket.on('Client-send-data', function(data) { //data la data cua nguoi dung gui len
        console.log(socket.id + " vua gui: " + data);
        //io.sockets.emit('Server-send-data', data + '888'); //server se gui all data cho all client
        //socket.emit('Server-send-data', data + '888'); //server chi gui ve chinh doi tuong dang chat
        socket.broadcast.emit('Server-send-data', data + '888'); //server se gui data ve all cac socket khac tru socket gui data. vi du: chi gui ve B C chu A khong nhan duoc
        // io.to("id cua nguoi nhan").emit(); //tim cach luu lai socket id cua all mn roi truyen vao do la duoc. vi du: a gui cho b, b gui cho a.
